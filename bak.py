...
def softmax(x):
    if x.ndim==2:
        x=x.T# 전치시키는 이유는 브로드 캐스트를 위해서임
        x=x-np.max(x,axis=0)# (10,3) 에서 하면 (10,)가 되는데, 브로드 캐스느는 열의 개수가 같아야지만 가능함. 그래서 전치를 시킨 것임.
...
def crosss_entropy(y_hat,real_y):
    ...
    return -np.sum(np.log(y_hat[np.arange(batch_size),real_y]+1e-7))/batch_size#sum에 axis가 없는 것을 보면.. 다 더해지는 듯 하구만.
...
class Mymodel():
    def __init__(self,input_size,hidden_size,output_size,activation='relu'):
        ...
        for idx in range(1,self.hidden_layer_num+1):
            ...
            self.layers['Activation_function'+str(idx)]=activation_layer[activation]#이걸로 해당 레이어에 액티베이션 함수를 넣는것..
        idx=self.hidden_layer_num+1
        self.layers['Layer'+str(idx)]=Layer(self.params['W'+str(idx)],self.params())
        self.last_layer=Softmax()#마지막 레이어
# 가중치 he초기화 , 사비에르 초기화.
